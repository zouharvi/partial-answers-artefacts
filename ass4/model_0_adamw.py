...
def create_model(X_train, Y_train):
    ...
    # SGD optimizer
    sgd = AdamW(lr=learning_rate, weight_decay=0.01)
    #sgd = SGD(lr=learning_rate, weight_decay=0.01)
...
def split_test_set_predict(model, embeddings, encoder, X_full, X_test, Y_test):
    ...
    incorrect = [
        (x, y_pred, y_test)
        for x, y_pred, y_test in zip(X_full, labels_pred, labels_test)
        if y_pred != y_test
    ]
    for x in incorrect[:100]:
        print(*x)
    
    print(len(incorrect), "incorrect samples out of", len(Y_pred), "total")
...
def main():
    ...
    _, _, X_test_text, _ = split_data(X_full, Y_full, args.test_percentage)
    ...
    split_test_set_predict(model, embeddings, encoder, X_test_text, X_test, Y_test)
