...
if __name__ == "__main__":
    ...
    if args.experiment == "main":
        ...
        scores = []
        for train_i, test_i in kf.split(X_full): # For each fold compute metrics
        ...
        data_out = {}
        for model_name, model_class in model_factory("all").items(): # For each relevant model
            for tf_idf in [False, True]: # Using Bow and TF-IDF
                ...
                accs = []
                for train_index in np.linspace(10, len(X_train), num=20): # For different amounts of train data
